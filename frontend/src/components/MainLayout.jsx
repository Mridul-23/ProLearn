...
const MainLayout = () => {
  ...
  // Find current active page label for header
  const currentMenu = MENU.find((m) => m.path === location.pathname) || { label: 'Dashboard' };
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 font-poppins text-slate-100">
          ...
          {isOpen && (
            <div className="pt-4 border-t border-slate-800/80 mt-auto">
              <div className="px-3.5 py-2.5 rounded-xl bg-slate-950/40 border border-slate-800/60 text-xs text-slate-400">
                <span className="font-medium text-slate-300 block">Pro Workspace</span>
                <span className="text-[10px] text-indigo-400 font-medium">AI Engine Active</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Dashboard Header */}
        <header className="h-20 bg-slate-900/40 backdrop-blur-md border-b border-slate-800/80 px-8 flex items-center justify-between sticky top-0 z-10">
          <div>
        ...
        {/* Page Content Outlet */}
        <main className="flex-1 p-8 overflow-x-hidden bg-slate-950">
          <Outlet />
        </main>
      </div>
    </div>
